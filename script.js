let cityInput = document.getElementById('city_input'),
    ...
    api_key = '8ad1477085a175d7579f2d27b737aa5a',
    ...
    fetch(WEATHER_API_URL).then(res => res.json()).then(data => {
        ...
        currentWeatherCard.innerHTML = `<div class="current-weather">
                    ...
                    <div class="card-footer">
                        <p><i class="fa-regular fa-calendar"></i> ${days[date.getDay()]}, ${date.getDate()},
                        ${months[date.getMonth() - 1]} , ${date.getFullYear()}</p>
    ...
    fetch(FORECAST_API_URL).then(res => res.json()).then(data => {
        ...
        for (let i = 1; i < fiveDaysForecast.length; i++) {
            ...
            <div class="forecast-items">
                ...
                <p>${date.getDate()} ${months[date.getMonth() - 1]}</p>
...
function getUserCoordinates(){
    navigator.geolocation.getCurrentPosition(position => {
        ...
        fetch(REVERSE_GEOCODING_URL).then(res => res.json()).then(data => {
            ...
            alert('Failed to connect coordinates');
        });
    }, error => {
        if(error.code === error.PERMISSION_DENIED){
            alert(` geogication premission denied. please reset location premission to grant again`);
        }
    } );
}
